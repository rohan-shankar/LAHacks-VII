...
def upload_file():
    ...
    relPathOfInputImage = os.path.join('static', 'uploads', file.filename)
    file.save(relPathOfInputImage)

    # Run the model
    undamaged_count, damaged_count, image = run_model(relPathOfInputImage)

    # Save the output image
    relPathOfOutputImage = os.path.join('static', 'out.png')
    cv2.imwrite(relPathOfOutputImage, image)

    # Return the results
    return {'undamaged_count': undamaged_count, 'damaged_count': damaged_count}

def run_model(relPathOfInputImage):
    damaged_count = 0
    undamaged_count = 0
    ...
    return undamaged_count, damaged_count, image
